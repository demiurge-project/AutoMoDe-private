...
function write_state() {
  ...
  echo "CLE$INDEX   \"--cle$INDEX \"  c (0,1,2,3,4,5,6) | as.numeric(S$INDEX) %in% c(0,1,2,3,4,5,6,7)" >> ${TXT_FILE}
  echo "CLR$INDEX   \"--clr$INDEX \"  c (1,2,3,4,5,6) | as.numeric(S$INDEX) %in% c(6,7)" >> ${TXT_FILE}
  if [ ${INDEX} == 0 ]; then
...
function write_connection() {
  ...
  echo "I${STATE}x$CONNECTION  \"--i${STATE}x$CONNECTION \" c   (1, 9, 15) | as.numeric(C${STATE}x$CONNECTION)==6 " >> ${TXT_FILE}
  ...
  echo "D${STATE}x$CONNECTION  \"--d${STATE}x$CONNECTION \" c   (30) | as.numeric(C${STATE}x$CONNECTION)==6 " >> ${TXT_FILE}
