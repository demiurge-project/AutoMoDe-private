...
function write_state() {
  ...
  echo "VEL$INDEX   \"--vel$INDEX \"  c (1.0) | as.numeric(S$INDEX) %in% c(8,9)" >> ${TXT_FILE}
...
function write_connection() {
  ...
  echo "C${STATE}x$CONNECTION  \"--c${STATE}x$CONNECTION \" c   (0,1,2,3,4,5,7) | as.numeric(NumConnections$STATE)>$CONNECTION " >> ${TXT_FILE}
  echo "P${STATE}x$CONNECTION  \"--p${STATE}x$CONNECTION \" r   (0,1) | as.numeric(C${STATE}x$CONNECTION) %in% c(0,1,2,5,7) " >> ${TXT_FILE}
  ...
  echo "L${STATE}x$CONNECTION  \"--l${STATE}x$CONNECTION \" c   (1,2,3,4,5,6) | as.numeric(C${STATE}x$CONNECTION)==7 " >> ${TXT_FILE}
